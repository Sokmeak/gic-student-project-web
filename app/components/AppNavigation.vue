  ...
  <nav
    ...
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
      ...
      <NuxtLink to="/" class="flex items-center shrink-0">
        <NuxtImg src="/gic-logo.jpg" alt="GIC Student Portal" width="150" height="40"
      ...
      <div class="hidden lg:flex items-center space-x-1 flex-1 mx-8">
        <NavLink v-for="item in navigationItems" :key="item.to" :to="item.to" :label="item.label"
      ...
      <div class="hidden md:block">
        <UserMenu :is-authenticated="isAuthenticated" :user="user" @logout="handleLogout" />
      </div>

      <!-- Mobile Menu Button -->
      <UButton icon="i-heroicons-bars-3-20-solid" color="gray" variant="ghost" size="md"
        class="lg:hidden text-neutral-700 dark:text-neutral-200" :ui="{ rounded: 'rounded-lg' }"
        @click="mobileMenuOpen = !mobileMenuOpen">
        <template v-if="mobileMenuOpen">
          <UIcon name="i-heroicons-x-mark-20-solid" class="text-neutral-700 dark:text-neutral-200" />
        </template>
      </UButton>
    </div>
    ...
    <Transition enter-active-class="transition ease-out duration-100"
      ...
      leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 translate-y-0"
      ...
      <div v-if="mobileMenuOpen"
        ...
        <div class="border-t border-gray-200 dark:border-neutral-800 py-4 space-y-2">
          <!-- Mobile Navigation Links -->
          <NavLink v-for="item in navigationItems" :key="item.to" :to="item.to" :label="item.label"
            :is-active="isActiveRoute(item.to)" mobile mobile-class="w-full px-4 py-3 rounded-lg"
